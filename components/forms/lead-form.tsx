...
export function LeadForm({kind}:{kind:LeadKind}) {
 const fields=kind==='vehicle'?[...vehicle,...contact]:[...contact.map(f=>f.name==='email'&&kind==='partnership'?{...f,required:true}:f),...(kind==='partnership'?partnership:[{name:'message',label:'How can we help?',required:true,type:'textarea',wide:true}])];
